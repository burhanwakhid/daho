...
class PostsComponentClient extends PostsComponent {
  ...
  void _applyBinding1() {
    _nodes.anchors[1]!.setNodes(ExpressionEvaluator.isTruthy(loading) ? _fragment1Then() : _fragment1Else());
  }
  ...
  void _applyBinding2() {
    _nodes.anchors[2]!.setNodes(ExpressionEvaluator.isTruthy(errorMessage != null) ? _fragment2Then() : _fragment2Else());
  }
  ...
  void _applyBinding4() {
    _nodes.anchors[4]!.setNodes(ExpressionEvaluator.isTruthy(posts.isNotEmpty) ? _fragment4Then() : _fragment4Else());
  }
...
void onInit() {loading = true; errorMessage = null; web.window.fetch('https://jsonplaceholder.typicode.com/posts'.toJS).toDart.then((response) {if (!response.ok) {throw Exception('HTTP ${response.status}');} return response.text().toDart;}).then((text) {final decoded = (jsonDecode(text.toDart) as List).cast<Map<String, dynamic>>(); posts = decoded.take(10).toList(); loading = false;}).catchError((e) {errorMessage = 'Failed to load posts: $e'; loading = false;});}
