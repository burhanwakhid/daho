...
void main() {
  group('SessionManager', () {
    ...
    test('construction succeeds regardless of SessionConfig.secure '
      ...
      expect(
        () => SessionManager(store, const SessionConfig()),
        returnsNormally,
      );
      expect(
    ...
    test(
      'createSession cookie carries HttpOnly and configured SameSite',
      () async {
        final req = buildRequest();
        final res = DahoResponse();
    ...
    test('loadSession returns the session for a valid cookie', () async {
      ...
      final loadReq = buildRequest(cookies: {'daho_session': created.id});
    ...
    test(
      'destroySession is a no-op (still clears cookie) when no session cookie present',
      () async {
        final req = buildRequest();
        final res = DahoResponse();
